...
public class TrailerBody : MonoBehaviour
{
    ...
    [Header("Grid")]
    [SerializeField] private TruckGrid trailerGrid;
    [SerializeField] private bool destroyPlacedBlocksOnBreak = true;

    [Header("Visual")]
    [SerializeField] private Color healthyColor = new Color(0.25f, 0.45f, 0.8f, 0.45f);
    [SerializeField] private Color damagedColor = new Color(1f, 0.25f, 0.1f, 0.7f);

    [Header("Anchor")]
    [SerializeField] private Vector2 frontAnchorOffset = new Vector2(1.5f, 0f);
    ...
    public static readonly System.Collections.Generic.List<TrailerBody> ActiveTrailers = new System.Collections.Generic.List<TrailerBody>();

    public float MaxHealth => maxHealth;
    public float CurrentHealth => currentHealth;
    public float HealthRatio => maxHealth <= 0f ? 0f : currentHealth / maxHealth;
    public bool IsDestroyed => currentHealth <= 0f;
    public Vector2 FrontAnchorOffset => frontAnchorOffset;
    ...
    private void Awake()
    {
        ...
        if (trailerGrid == null)
        {
            trailerGrid = GetComponentInChildren<TruckGrid>();
        }
    ...
    {
        if (!ActiveTrailers.Contains(this)) ActiveTrailers.Add(this);
    }

    private void OnDisable()
    {
        ActiveTrailers.Remove(this);
    }
    ...
    private void BreakTrailer()
    {
        if (trailerGrid != null)
        {
            trailerGrid.ClearPlacedBlocks(destroyPlacedBlocksOnBreak);
        }
    ...
    private void OnDrawGizmos()
    {
        ...
        Gizmos.DrawWireSphere(frontAnchorOffset, 0.15f);
        Gizmos.DrawLine(Vector3.zero, frontAnchorOffset);

        // Rear Anchor (Red)
        Gizmos.color = Color.red;
        Gizmos.DrawWireSphere(rearAnchorOffset, 0.15f);
        Gizmos.DrawLine(Vector3.zero, rearAnchorOffset);
